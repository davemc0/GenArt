...
namespace {
    ...
    std::string ExpressionsToTest[] = {
        ...
        "* sqr y y",
        "* x / 1 y",
        "+ % * ln clamp atan * x 0.164251 % pow x 0.964193 clamp sqrt y pow abs ln + y x 0.27132 ln acos ln - / sin exp exp - sin + y x - tan x x exp sin exp exp - ^ cos x - x 0.650258 y - sin exp exp sqr x sin exp exp sin sqrt y",
        "+ + + atan * r clamp * 0.759335 r atan * ln * 0.4343 ln ln r ifs acos r + abs + r * x y * 0.4343 ln % x y ifs + y + 0.266561 ~ r + -0.247315 * 0.4343 ln - 0.446822 / x y ifs + y + 0.323163 ~ r + sin cos y * 0.4343 ln - 0.501776 / x y",
        "+ + + atan * r clamp * 0.759335 r atan * ln * 0.4343 ln ln r ifs acos r + abs + r * x y * 0.4343 ln % x y ifs + y + 0.323163 ~ r + sin cos y * 0.4343 ln - 0.501776 / x y ifs + y + 0.266561 ~ r + -0.247315 * 0.4343 ln - 0.446822 / x y",
        "+ - exp ^ % asin r * 4.06077 y + -0.200677 + y asin asin r / * atan x sin clamp - 0.195316 x ~ r * 0.5 sqrt tan + exp sin exp exp - -0.857132 y exp - sin + x sin exp exp + -0.706627 + x sin x ln cube atan abs * 0.0364843 ^ x y",
        ...
        "- * sqrt tan exp - * clamp sin tan - cube acos pow r 0.25 * 0.4343 ln cbrt % y x cos y asin pow 0.330428 * 1.27142 cos y ^ * exp - sin x asin ^ sin y ifs | 0.283943 asin y + sqr y * 0.4343 ln - 0.501776 / x y sin * 0.389718 asin y cos y ifs + exp round r + 0.179928 ln * sqrt sin & 0.71179 y + -0.720325 sin r + sqr y * 0.4343 ln - 0.499015 / x y",
        "- - x x clamp 0.864688",
    ...
    public:
        GetTestExprFromList(VarVals_t vvals_) : vvals(vvals_)
        {
            index = 0;
        }
        ...
        Expr* operator()()
        {
            if (index >= sizeof(ExpressionsToTest) / sizeof(std::string))
                return NULL;
    ...
    public:
        ...
        Expr* operator()()
        {
            if (index > max_exprs && max_exprs > 0)
                return NULL;
    ...
    public:
        ...
        double operator()(Expr* A, Expr* B)
        {
            ...
            for (int i = 0; i < numSamples; i++) {
                for (size_t j = 0; j < vvals.vals.size(); j++)
                    vvals.vals[j] = i == 0 ? 0.0f : DRandf();
                vvals.vals[2] = sqrtf(vvals.vals[0] * vvals.vals[0] + vvals.vals[1] * vvals.vals[1]);
...
//             opI.spans[1] = interval(0,1);
            ...
            if(!fits) {
                std::cerr << "FAIL: " << tostring(opI.spans[0]) << '\t' << tostring(opI.spans[1]) << "\tI: "
                    << tostring(ival) << " S: " << tostring(sival) << " = " << A->Print(PREFIX) << '\n';
#if 0
    ...
    {
    public:
        ResultActionLogFile(bool PrintErrors_, bool PrintAll_, std::ofstream& GoodEqnFile_, std::ofstream& BadEqnFile_,
            std::ofstream& GoodOptEqnFile_, std::ofstream& BadOptEqnFile_)
            : maxFailPercent(0), m_tested(0), m_failed(0), m_printErrors(PrintErrors_), m_printAll(PrintAll_),
            m_goodEqnFile(GoodEqnFile_), m_badEqnFile(BadEqnFile_), m_goodOptEqnFile(GoodOptEqnFile_), m_badOptEqnFile(BadOptEqnFile_)
        {}
        ...
        void operator()(Expr* A, Expr* B, double failingPercent)
        {
            ...
            if (!passed) {
                m_failed++;
                if (m_printErrors)
                    std::cerr << "Mismatch percentage: " << failingPercent << "\n";
            }

            if ((!passed && m_printErrors) || m_printAll)
                std::cerr << "A: " << A->Print(INFIX) << "\nB: " << B->Print(INFIX) << '\n';
...
template<typename GTE, typename TI, typename SL, typename RA>
void ExprOptTestHarness(GTE GetEx, TI TestItem, SL SamplerLoop, RA ResultAction)
{
    for (Expr* A; A = GetEx();) {
