...
void CopyVValsToArray(const VarVals_t *VV, float *VVals)
{
    ...
    for (size_t i = 0; i < VV->vals.size(); i++)
        VVals[i] = VV->vals[i];
}
...
namespace {
    ...
    public:
        ...
        void init()
        {
            ...
            for(int n=0; n<sizeof(numerators)/sizeof(float); n++) {
                denom(numerators[n]);
            }
...
interval sampleIval(const Expr* E, const opInfo &opI, const interval &lv, const interval &rv)
{
    ...
    if (hasV & 2) { xSS.insert_span(opI.spans[1], 1); xSS.insert_span(-opI.spans[1], 1); }
    if (hasV & 4) { xSS.insert_span(opI.spans[2], 1); xSS.insert_span(-opI.spans[2], 1); }
    SampleSet ySS((hasV & 2) ? interval(opI.spans[1]) : interval(), opI.steps);
    if (hasV & 1) { ySS.insert_span(opI.spans[0], 1); ySS.insert_span(-opI.spans[0], 1); }
    if (hasV & 4) { ySS.insert_span(opI.spans[2], 1); ySS.insert_span(-opI.spans[2], 1); }
    SampleSet rSS((hasV & 4) ? interval(opI.spans[2]) : interval(), opI.steps);
    if (hasV & 1) { rSS.insert_span(opI.spans[0], 1); rSS.insert_span(-opI.spans[0], 1); }
    if (hasV & 2) { rSS.insert_span(opI.spans[1], 1); rSS.insert_span(-opI.spans[1], 1); }
    ...
    for(float x : xSS.m_table) {
        ...
        for(float y : ySS.m_table) {
            ...
            for(float r : rSS.m_table) {
                ...
                if (opI.stopAtMaxErr && iout.span() >= opI.maxAbsErr)
                    break;
            }
        }
    }
...
Expr* Optimize(const Expr* E, const VarVals_t &MinVV, const VarVals_t &MaxVV, const int steps, const float maxAbsErr, const interval outSpan)
{
    ...
    opInfo opI;
    opI.vn = MinVV;
    for (int phase=0; phase<VarVals_t::NUM_VARS; phase++)
        opI.spans[phase] = interval(MinVV.vals[phase], MaxVV.vals[phase]);
    opI.steps = steps;
    ...
    if (F->ivl.upper <= outSpan.lower)
        F->ivl = interval(outSpan.lower);
    if (F->ivl.lower >= outSpan.upper)
        F->ivl = interval(outSpan.upper);
...
{
    if (typeid(*E) == typeid(Var) && ((const Var *)E)->getVarID() == size_t(VarID))
        return new Const(val);

    Expr* L = E->left ? ReplaceVarWithConst(E->left, VarID, val) : NULL;
    if (L && L != E->left) { delete E->left; E->left = L; }
    Expr* R = E->right ? ReplaceVarWithConst(E->right, VarID, val) : NULL;
    if (R && R != E->right) { delete E->right; E->right = R; }
