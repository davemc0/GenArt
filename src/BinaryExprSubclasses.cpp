...
bool BinaryExpr::isequal(const Expr* E) const
{
    ...
    if (left->isequal(E->left) && right->isequal(E->right))
        return true;

    if (isSymmetric() && left->isequal(E->right) && right->isequal(E->left)) {
        return true;
    }
...
{
    Expr* L = left->Opt(opI);
    if (L && L != left) { L->ivl = left->ivl; delete left; left = L; }
    Expr* R = right->Opt(opI);
    if (R && R != right) { R->ivl = right->ivl; delete right; right = R; }
        ...
        if (FOG(SE)) {
            ...
            if (typeid(*left) != typeid(*this) && typeid(*right) != typeid(*this)) {
                ...
                if (typeid(*left) == typeid(*this)) {
                    ...
                    while (typeid(*R->right) == typeid(*this))
                        R = R->right;
...
Expr* BinaryExpr::Mutate(const int prob, const int siz, const float ConstPerturb, const VarVals_t *VV)
{
    ...
    if (L && L != left) { delete left; left = L; }

    Expr* R = right->Mutate(prob, siz, ConstPerturb, VV);
    if (R && R != right) { delete right; right = R; }
    ...
    case 2: // Make self a child of a new one
        if (chance(1,2)) {
            ...
            return chance(1,2) ? RandBinaryNode(Copy(), RandExpr(siz, VV)) :
                RandBinaryNode(RandExpr(siz, VV), Copy());
        }
...
std::string And::Print(int pstyle) const
{
    ...
    if (pstyle & (OP_EVAL|OP_EVAL_IMPURE)) {
        ...
        return (lp?"(":"") + left->Print(pstyle) + (lp?")":"") +
            " " + getName() + " " + (rp?"(":"") + right->Print(pstyle) + (rp?")":"");
    }
}
...
Expr* And::Opt(const opInfo &opI)
{
    ...
    if (left->ivl.lower >= 1.0f && FOG(AA))
        return new Clamp(GrabR()); // 1&A => A
    if (right->ivl.lower >= 1.0f && FOG(AA))
        return new Clamp(GrabL()); // A&1 => A

    // A&A => A
    if (left->isequal(right) && FOG(AA))
        return GrabL();

    // A&(A|B) => A
    if (typeid(*right) == typeid(Or) && left->isequal(right->left) && FOG(AA))
      return GrabL();

    // ~A&A => 0
    if (typeid(*left) == typeid(BitNot) && left->left->isequal(right) && FOG(AA))
      return new Const(0);

    // (B&A)&B => B&A
    // (A&B)&B => A&B
    if (typeid(*left) == typeid(And) && (left->left->isequal(right) || left->right->isequal(right)) && FOG(AA))
        return GrabL();
    // B&(B&A) => B&A
    // B&(A&B) => A&B
    if (typeid(*right) == typeid(And) && (right->left->isequal(left) || right->right->isequal(left)) && FOG(AA))
        return GrabR();
    // (~A)&(~B) => ~(A|B)
    if (typeid(*left) == typeid(BitNot) && typeid(*right) == typeid(BitNot) && FOG(AA))
        return new BitNot(new Or(left->GrabL(), right->GrabL()));
    ...
    // (A&B)&C => B&(A&C)
    if (typeid(*left) == typeid(And) && FOG(NE))
        return new And(left->GrabR(), new And(GrabR(), left->GrabL()));
    // A&(B&C) => (C&A)&B
    if (typeid(*right) == typeid(And) && FOG(NE))
        return new And(new And(right->GrabL(), GrabL()), right->GrabR());

    // Clamp(A)&B => A&B because & clamps both its inputs.
    if (typeid(*left) == typeid(Clamp) && FOG(AB))
        return new And(left->GrabL(), GrabR());
    if (typeid(*right) == typeid(Clamp) && FOG(AB))
        return new And(GrabL(), right->GrabL());
...
Expr* ATan2::Opt(const opInfo& opI)
{
    ...
    if(left->isequal(right)) {
        if (left->ivl.lower >= 0 && right->ivl.lower >= 0 && FOG(AA))
            return new Const(atan2f(1.0f,1.0f));
        if (left->ivl.upper <= 0 && right->ivl.upper <= 0 && FOG(AA))
            return new Const(atan2f(-1.0f,-1.0f));
...
Expr* Div::Opt(const opInfo& opI)
{
    ...
    // (A/B)/C => A/(B*C)
    if (typeid(*left) == typeid(Div) && FOG(AB))
        return new Div(left->GrabL(), new Mult(left->GrabR(), GrabR()));
    // A/(B/C) => (A*C)/B
    if (typeid(*right) == typeid(Div) && FOG(AB))
        return new Div(new Mult(GrabL(), right->GrabR()), right->GrabL());

    // (A*B)/A => B
    if (typeid(*left) == typeid(Mult) && left->left->isequal(right) && FOG(AA))
        return left->GrabR();
    // (A*B)/B => A
    if (typeid(*left) == typeid(Mult) && left->right->isequal(right) && FOG(AA))
        return left->GrabL();
    // A/(A*B) => 1/B
    if (typeid(*right) == typeid(Mult) && left->isequal(right->left) && FOG(AA))
        return new Div(new Const(1.0f), right->GrabR());
    // A/(B*A) => 1/B
    if (typeid(*right) == typeid(Mult) && left->isequal(right->right) && FOG(AA))
        return new Div(new Const(1.0f), right->GrabL());
    ...
    if (typeid(*right) == typeid(Const)) {
        ...
        if (FOG(AB))
            return new Mult(GrabL(), new Const(1.0f / rv)); // A/k1 => A*(1/k1)
    }

    // A/A => 1
    if (left->isequal(right) && FOG(AA))
        return new Const(1.0f);
    ...
    // (-A)/B => -(A/B)
    if (typeid(*left) == typeid(UnaryMinus) && FOG(NL))
        return new UnaryMinus(new Div(left->GrabL(), GrabR()));
    // A/(-B) => -(A/B)
    if (typeid(*right) == typeid(UnaryMinus) && FOG(NL))
        return new UnaryMinus(new Div(GrabL(), right->GrabL()));

    // (C*A)/B => (A/B)*C
    if (typeid(*left) == typeid(Mult) && FOG(NE))
        return new Mult(new Div(left->GrabR(), GrabR()), left->GrabL());
...
Expr* Minus::Opt(const opInfo& opI)
{
    ...
    // A-(A+B) => -B
    if (typeid(*right) == typeid(Plus) && left->isequal(right->left) && FOG(AA))
        return new UnaryMinus(right->GrabR());

    // A-(B+A) => -B
    if (typeid(*right) == typeid(Plus) && left->isequal(right->right) && FOG(AA))
        return new UnaryMinus(right->GrabL());

    // (A+B)-A => B
    if (typeid(*left) == typeid(Plus) && left->left->isequal(right) && FOG(AA))
        return left->GrabR();

    // (A+B)-B => A
    if (typeid(*left) == typeid(Plus) && left->right->isequal(right) && FOG(AA))
        return left->GrabL();

    // (A-B)-C => A-(B+C)
    if (typeid(*left) == typeid(Minus) && FOG(AB))
        return new Minus(left->GrabL(), new Plus(left->GrabR(), GrabR()));

    // A-(-B) = A+B
    if (typeid(*right) == typeid(UnaryMinus) && FOG(AB))
        return new Plus(GrabL(), right->GrabL());
    ...
    // (ln A) - (ln B) => ln(A/B)
    if (typeid(*left) == typeid(Ln) && typeid(*right) == typeid(Ln) && FOG(AA)) {
       return new Ln(new Div(left->GrabL(), right->GrabL()));
    }
    ...
    // (-A)-B = -(A+B)
    if (typeid(*left) == typeid(UnaryMinus) && FOG(NL))
        return new UnaryMinus(new Plus(left->GrabL(), GrabR()));
    ...
    // (A-B) => A+(-B)
    if (FOG(WE))
        return new Plus(GrabL(), new UnaryMinus(GrabR()));
...
Expr* Mod::Opt(const opInfo& opI)
{
    ...
    // A % -B => A%B
    if (typeid(*right) == typeid(Abs) && FOG(AB))
        return new Mod(GrabL(), right->GrabL());
    if (typeid(*right) == typeid(UnaryMinus) && FOG(AB))
        return new Mod(GrabL(), right->GrabL());
    ...
    if (lva.upper < rva.lower && !right->ivl.contains(0) && FOG(AI))
        return GrabL();
...
Expr* Mult::Opt(const opInfo& opI)
{
    ...
    // (A/B)*B => A
    if (typeid(*left) == typeid(Div) && left->right->isequal(right) && FOG(AA))
        return left->GrabL();

    // A*(B/A) => B
    if (typeid(*right) == typeid(Div) && left->isequal(right->right) && FOG(AA))
        return right->GrabL();
    ...
    // A*A => Sqr(A)
    if (left->isequal(right) && FOG(AA))
        return new Sqr(GrabL());

    // Sqr(A)*Sqr(B) => Sqr(A*B)
    if (typeid(*left) == typeid(Sqr) && typeid(*right) == typeid(Sqr) && FOG(AL))
        return new Sqr(new Mult(left->GrabL(), right->GrabL()));

    // (A/B)*A => Sqr(A)/B
    if (typeid(*left) == typeid(Div) && left->left->isequal(right) && FOG(AA))
        return new Div(new Sqr(GrabR()), left->GrabR());

    // A*(A/B) => Sqr(A)/B
    if (typeid(*right) == typeid(Div) && left->isequal(right->left) && FOG(AA))
        return new Div(new Sqr(GrabL()), right->GrabR());
    ...
    // (A*B)*C => B*(A*C)
    if (typeid(*left) == typeid(Mult) && FOG(NE))
        return new Mult(left->GrabR(), new Mult(GrabR(), left->GrabL()));
    // A*(B*C) => (C*A)*B
    if (typeid(*right) == typeid(Mult) && FOG(NE))
        return new Mult(new Mult(right->GrabL(), GrabL()), right->GrabR());
    ...
    // (A/B)*C => (C*A)/B
    if (typeid(*left) == typeid(Div) && FOG(NL))
        return new Div(new Mult(GrabR(), left->GrabL()), left->GrabR());
    // A*(B/C) => (B*A)/C
    if (typeid(*right) == typeid(Div) && FOG(NL))
        return new Div(new Mult(right->GrabL(), GrabL()), right->GrabR());

    // -A*B => -(A*B)
    if (typeid(*left) == typeid(UnaryMinus) && FOG(NL))
        return new UnaryMinus(new Mult(GrabR(), left->GrabL()));
    // A*-B => -(A*B)
    if (typeid(*right) == typeid(UnaryMinus) && FOG(NL))
        return new UnaryMinus(new Mult(right->GrabL(), GrabL()));
    ...
    if (typeid(*right) == typeid(Ln) && FOG(XX/*NE*/))
        return new Ln(new Pow(right->GrabL(), GrabL()));
...
Expr* Or::Opt(const opInfo& opI)
{
    ...
    if (left->ivl.upper <= 0.0f && FOG(AI))
        return new Clamp(GrabR()); // 0|A => A
    if (right->ivl.upper <= 0.0f && FOG(AI))
        return new Clamp(GrabL()); // A|0 => A
    ...
    // A|(A&B) => A
    if (typeid(*right) == typeid(And) && left->isequal(right->left) && FOG(AA))
      return GrabL();
    ...
    // (B|A)|B = B|A
    // (A|B)|B = A|B
    if (typeid(*left) == typeid(Or) && (left->left->isequal(right) || left->right->isequal(right)) && FOG(AA))
        return GrabL();
    // B|(B|A) = B|A
    // B|(A|B) = A|B
    if (typeid(*right) == typeid(Or) && (right->left->isequal(left) || right->right->isequal(left)) && FOG(AA))
        return GrabR();
    // (A|B)|(B|A) = A|B
    if (typeid(*left) == typeid(Or) && typeid(*right) == typeid(Or)
        && (right->left->isequal(left->right) || right->right->isequal(left->left)) && FOG(AA))
        return GrabL();
    // (~A)|(~B) = ~(A&B)
    if (typeid(*left) == typeid(BitNot) && typeid(*right) == typeid(BitNot) && FOG(AB))
        return new BitNot(new And(left->GrabL(), right->GrabL()));
    ...
    // Clamp(A)|B = A|B because | clamps both its inputs.
    if (typeid(*left) == typeid(Clamp) && FOG(AB))
        return new Or(left->GrabL(), GrabR());
    if (typeid(*right) == typeid(Clamp) && FOG(AB))
        return new Or(GrabL(), right->GrabL());

    // (A|B)|C => B|(A|C)
    if (typeid(*left) == typeid(Or) && FOG(NE))
        return new Or(left->GrabR(), new Or(GrabR(), left->GrabL()));
    // A|(B|C) => (C|A)|B
    if (typeid(*right) == typeid(Or) && FOG(NE))
        return new Or(new Or(right->GrabL(), GrabL()), right->GrabR());
...
Expr* Plus::Opt(const opInfo& opI)
{
    ...
    // (A+(B-A)) => B
    if (typeid(*right) == typeid(Minus) && left->isequal(right->right) && FOG(AA))
        return right->GrabL();

    // ((A-B)+B) => A
    if (typeid(*left) == typeid(Minus) && right->isequal(left->right) && FOG(AA))
        return left->GrabR();

    // (-A) + (-B) => -(A+B)
    if (typeid(*left) == typeid(UnaryMinus) && typeid(*right) == typeid(UnaryMinus) && FOG(AB))
        return new UnaryMinus(new Plus(left->GrabL(), right->GrabL()));

    // (ln A) + (ln B) => ln(A*B)
    if (typeid(*left) == typeid(Ln) && typeid(*right) == typeid(Ln) && FOG(AA))
        return new Ln(new Mult(left->GrabL(), right->GrabL()));

    // (A+A) => A*2
    if (left->isequal(right) && FOG(AA))
        return new Mult(GrabL(), new Const(2.0f));

    // ((B-A)+B) => B*2 - A
    if (typeid(*left) == typeid(Minus) && right->isequal(left->left) && FOG(AA))
        return new Minus(new Mult(left->GrabL(), new Const(2.0f)), left->GrabR());

    // (A+(A-B)) => A*2-B
    if (typeid(*right) == typeid(Minus) && left->isequal(right->left) && FOG(AA))
        return new Minus(new Mult(GrabL(), new Const(2.0f)), right->GrabR());
    ...
    // (-A)+B => B-A
    if (typeid(*left) == typeid(UnaryMinus) && FOG(AL))
        return new Minus(GrabR(), left->GrabL());
    // A+(-B) => A-B
    if (typeid(*right) == typeid(UnaryMinus) && FOG(AL))
        return new Minus(GrabL(), right->GrabL());

    // (A+B)+C => (B+C)+A
    if (typeid(*left) == typeid(Plus) && FOG(NE))
        return new Plus(left->GrabR(), new Plus(GrabR(), left->GrabL()));
    // A+(B+C) => (C+A)+B
    if (typeid(*right) == typeid(Plus) && FOG(NE))
        return new Plus(new Plus(right->GrabR(), GrabL()), right->GrabL());
    // A+(B-C) => (A+B)-C
    if (typeid(*right) == typeid(Minus) && FOG(NE))
        return new Minus(new Plus(GrabL(), right->GrabL()), right->GrabR());
...
Expr* Pow::Opt(const opInfo& opI)
{
    ...
    // (A^B)^C => A^(B*C)
    if (typeid(*left) == typeid(Pow) && FOG(AB))
        return new Pow(left->GrabL(), new Mult(left->GrabR(), GrabR()));
...
Expr* XOr::Opt(const opInfo& opI)
{
    ...
    if (left->ivl.lower >= 1.0f && FOG(AB))
        return new BitNot(GrabR()); // 1 xor A => ~A
    if (right->ivl.lower >= 1.0f && FOG(AB))
        return new BitNot(GrabL()); // A xor 1 => ~A

    if (left->ivl.upper <= 0.0f && FOG(AB))
        return new Clamp(GrabR()); // 0 xor A => A
    if (right->ivl.upper <= 0.0f && FOG(AB))
        return new Clamp(GrabL()); // A xor 0 => A

    // ~A xor ~B => A xor B
    if (typeid(*left) == typeid(BitNot) && typeid(*right) == typeid(BitNot) && FOG(AA)) 
        return new XOr(left->GrabL(), right->GrabL());

    // Clamp(A) xor B = A xor B because xor clamps both its inputs.
    if (typeid(*left) == typeid(Clamp) && FOG(AB))
        return new XOr(left->GrabL(), GrabR());
    if (typeid(*right) == typeid(Clamp) && FOG(AB))
        return new XOr(GrabL(), right->GrabL());
    ...
    if (typeid(*left) == typeid(XOr) && FOG(NE))
        return new XOr(left->GrabR(), new XOr(GrabR(), left->GrabL()));

    // A xor (B xor C) => (C xor A) xor B
    if (typeid(*right) == typeid(XOr) && FOG(NE))
        return new XOr(new XOr(right->GrabL(), GrabL()), right->GrabR());

    // ~A xor B => ~(A xor B)
    if (typeid(*left) == typeid(BitNot) && FOG(NL))
      return new BitNot(new XOr(left->GrabL(), GrabR()));
