...
void MathStyle::LoadPopulation(std::string& inFName)
{
...
Expr* MathStyle::BreedChannel(const MathIndividual::shp Aa, const MathIndividual::shp Bb, int chan, ColorSpace_t ColorSpace, const VarVals_t* VV)
{
    ...
    if (chanChoices.size() == 0 || chance(0.25f * m_variability)) {
		C->inc("Chan.RandExpr");
		if (chanChoices.size() > 0)
			C->inc("Chan.RandExpr.One");
	...
	if (chanChoices.size() == 1 || chance(1.0f - m_variability)) {
		...
		switch (int(c)) {
		case 0:
			C->inc("Chan.One.MutateExpr");
			return MutateExpr(M, MUTPROB, MUTSIZE, CONST_PERTURB, VV);
		case 1:
			C->inc("Chan.One.PerturbConstants");
			return M->Copy()->PerturbConstants(CONST_PERTURB);
        default:
			C->inc("Chan.One.Copy");
			return M->Copy();
		}
...
ColorMap<f3Pixel>* MathStyle::BreedColorMap(MathIndividual::shp Aa, MathIndividual::shp Bb, ColorSpace_t ColorSpace)
{
	...
	if (ColorSpace == SPACE_COLMAP || ColorSpace == SPACE_TONEMAP_COLMAP) {
        ...
        if (cmapChoices.size() == 0 || chance(0.25f * m_variability)) {
			C->inc("CMap.Random");
			if(cmapChoices.size() > 0)
				C->inc("CMap.Random.One");
...
Individual::shp MathStyle::BreedIndividual(int IDNum, int Generation, Individual::shp A_, Individual::shp B_, float XMin, float YMin, float BoxWid)
{
	...
	if (Bb && chance(1.0f - m_variability))
		ColorSpace = Bb->ColorSpace;
	if (Aa && chance(1.0f - m_variability))
        ColorSpace = Aa->ColorSpace;
    ...
    MathIndividual::shp newInd(new MathIndividual(R, G, B, CMap, ColorSpace, 
        DRandf(0.0f, 0.01f), IDNum,
        Generation, Aa ? Aa->IDNum : -1, Bb ? Bb->IDNum : -1, XMin, YMin, BoxWid));
...
{
    float T = 0.0f;
    for (size_t i = 0; i < CMap.size() - 1; i++) {
        T += L2Norm(CMap[i] - CMap[i + 1]);
    }
...
void FillColorMapRandom(ColorMap<f3Pixel> &CMap)
{
	for (size_t i = 0; i < CMap.size(); i++)
		CMap[i] = f3Pixel(DRandf(), DRandf(), DRandf());
}
    ...
    if (Siz > 2) {
        ...
        for (int j = 0; j < 100; j++) {
            ...
            for (size_t i = 0; i<Siz*Siz * 20; i++) {
                ...
                if ((CMapReorderStyle == REORDER_SHORT && (D0 + D1 > D1 + D2)) ||
                    (CMapReorderStyle == REORDER_LONG && (D0 + D1 < D1 + D2))) {
            ...
            if ((CMapReorderStyle == REORDER_SHORT && (Score < BestScore)) ||
                (CMapReorderStyle == REORDER_LONG && (Score > BestScore))) {
                    BestScore = Score;
                    BestCMap = CMap;
            }
...
ColorMap<f3Pixel> GetColorMapFromImage(const char *imgFName, int colorMapSize)
{
    ...
    Quantizer<uc3Pixel, unsigned char> Qnt(SampImg.pp(), SampImg.size());
    Qnt.SetParams(CMap.size(), true, 64);
...
ColorMap<f3Pixel> * MutateColorMap(const ColorMap<f3Pixel> &Aa, float variability)
{
	...
	for (size_t i = 0; i < Sz; i++)
		(*M)[i] = f3Pixel(
		dmcm::Saturate(Aa[i].r() + DRandf(-p[i], p[i])),
		dmcm::Saturate(Aa[i].g() + DRandf(-p[i], p[i])),
		dmcm::Saturate(Aa[i].b() + DRandf(-p[i], p[i])));
