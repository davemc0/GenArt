...
extern void InvokeRenderKernel(uchar4* uc4DevPtr, size_t pitch, int wid, int hgt, int GridHgtInPix, int yofs, float BoxWid, float Xmin, float Ymin,
                               float MinSamples, cudaTextureObject_t ColMapTexObj, ColorSpace_t ColorSpace);

extern cudaTextureObject_t loadColorMapTexture(cudaArray* ColMapArray);
