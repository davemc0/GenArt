...
// An IndivID is based on the time in seconds of the first call to this function plus a serial number offset.
...
std::string Individual::stringSave()
{
    ...
    outStream << GetScore() << " " << GetIDNum() << " " << GetGeneration()
        << " " << GetParentA() << " " << GetParentB()
        << " " << Xmin << " " << Ymin << " " << BoxWid;
...
std::string Individual::stringDisplay(const float x /*= -1.0f*/, const float y /*= -1.0f*/)
{
    ...
    FloatFmt(out, 2);
    out << "IDNum=" << IDNum << " Score=" << Score
        << " Corner=" << Xmin << "," << Ymin << " BoxWid=" << BoxWid
        << " Times=" << ThumbImD()->renderTime() << ", " << FinalImD()->renderTime()
