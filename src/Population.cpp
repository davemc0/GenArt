...
{
    Zoo.push_back(Parents[i]);
    Parents.erase(beginParents() + i);
}

void Population::MoveChildToZoo(size_t i)
{
    Zoo.push_back(Children[i]);
    Children.erase(beginChildren() + i);
}
...
{
    Parents.push_back(Zoo[i]);
    Zoo.erase(beginZoo() + i);
}

void Population::ClearImages()
{
    for (auto ind = beginParents(); ind != endParents(); ind++) (*ind)->ImClear();
    for (auto ind = beginChildren(); ind != endChildren(); ind++) (*ind)->ImClear();
    for (auto ind = beginZoo(); ind != endZoo(); ind++) (*ind)->ImClear();
}
...
void Population::DeleteSpanOfIndividuals(size_t index, size_t count, bool AmParent, bool AmChild)
{
    ...
    if (AmParent) {
        if (Last > sizeParents()) Last = sizeParents();
        if (index < Last && index < sizeParents()) Parents.erase(beginParents() + index, beginParents() + Last);
    } else if (AmChild) {
        if (Last > sizeChildren()) Last = sizeChildren();
        if (index < Last && index < sizeChildren()) Children.erase(beginChildren() + index, beginChildren() + Last);
    } else {
        if (Last > sizeZoo()) Last = sizeZoo();
        if (index < Last && index < sizeZoo()) Zoo.erase(beginZoo() + index, beginZoo() + Last);
    }
}
...
void Population::SavePopulation(const std::string& outFNameBase, Style* style)
{
    try {
        ...
        if (!outStream.is_open()) throw DMcError("SavePopulation: Couldn't open file " + outFName);

        std::cerr << "Saving Parents and Zoo to " << outFName << '\n';

        for (auto ind = beginParents(); ind != endParents(); ind++) outStream << (*ind)->stringSave();
        for (auto ind = beginZoo(); ind != endZoo(); ind++) outStream << (*ind)->stringSave();
